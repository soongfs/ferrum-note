# FerrumNote 中文说明

English README: [README.md](./README.md)

本页是中文入口，完整文档以英文 README 为主。
FerrumNote 借鉴 Typora 的交互体验，但为独立项目，与 Typora 无隶属关系。

## 快速入口

- 项目主页说明： [README.md](./README.md)
- 架构文档： [docs/architecture.md](./docs/architecture.md)
- 引擎架构： [docs/engine-architecture.md](./docs/engine-architecture.md)
- MVP 范围： [docs/mvp-scope.md](./docs/mvp-scope.md)
- 发布指南： [docs/release-guide.md](./docs/release-guide.md)
- WSL 排障： [docs/wsl-troubleshooting.md](./docs/wsl-troubleshooting.md)

## 当前阶段重点

- 左侧工作区资源管理器 + 右侧编辑区 + 底部状态栏
- 工作区通过 `Open Folder` 设置并持久化到 `~/.ferrumnote/config.toml`
- Writer 模式切到自研引擎路线：Rust core + WASM + 自定义编辑面
- Source 模式继续由 CodeMirror 负责源码编辑
- Rust 引擎变更后需要执行 `pnpm engine:build` 更新 `src/engine/pkg`
- 浏览器模式显式降级，禁用桌面文件能力
